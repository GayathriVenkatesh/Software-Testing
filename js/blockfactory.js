...
class SVG {
    ...
    setClampSlots(clamp, number) {
        if (clamp > this._clampSlots.length - 1) {
     ...
     * @returns {void}
     */ _calculateXY() {
        let x = this._strokeWidth / 2.0; // 0.5
        let y = this._strokeWidth / 2.0 + this._radius; // 8.5
        this.margins[0] = x + this._strokeWidth + 0.5;  // 2
        this.margins[1] = this._strokeWidth + 0.5; // 1.5

        if (this._outie) {
            x += this._innieX1 + this._innieX2; // 8.5
            this.margins[0] += this._innieX1 + this._innieX2; // 10
        }

        if (this._cap) {
            y += this._slotY * 3.0;  // y = 14.5
            this.margins[1] += this._slotY * 3.0; // 7.5
        } else if (this._slot) {
            this.margins[1] += this._slotY; // 3.5
        }
